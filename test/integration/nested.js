...
describe('nested case', function () {
  ...
  it('nested dep', function (done) {
    webpack(webpackConfig2, function (err, stats) {
      ...
      expect(warns[0]).to.include('[webpack-transform-modules-plugin] There are multiple node modules configured "transformModules" with module: "a"')
      expect(warns[0]).to.include('/webpack-transform-modules-plugin/test/cases/nested-dep/node_modules/d')
      expect(warns[1]).to.include('[webpack-transform-modules-plugin] There are multiple node modules configured "transformModules" with module: "a"')
      expect(warns[1]).to.include('/webpack-transform-modules-plugin/test/cases/nested-dep/node_modules/b/node_modules/a')
      done()
    })
  })
})
